...
class HSBountyBot(HSBot):
    ...
    def _bounty(self,bounty_no):
        is_bounty =  self.retry_to_find_locations(self.hsmatch.is_bounty)
        if is_bounty == []:
            raise Exception("can't find the boss selection page" )
        locations =  self.retry_to_find_locations(self.hscontonur.list_bounties)
        ...
        start_loc =  self.retry_to_find_locations(self.hsmatch.find_bounty_start_choose)
        if start_loc == []:
    ...
    def _mercenary(self,mercenary_no):
        ...
        if next_step_action == "find_map_play":
            self.hssetting.debug_msg("let the map bot to handle")
            # self.click(button_location[0][0],button_location[0][1],random.randint(1,5),random.randint(1,5),sleep_time = 0.5)
            time.sleep(5)
